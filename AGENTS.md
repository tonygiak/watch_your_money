...
| **Current sprint**          | None active — **S-010 (discovery, `receipt-format-scope-and-auth-modernization`)** just closed. Three ADRs accepted (ADR-0014 receipt-format scope, ADR-0015 asymmetric JWT verification, ADR-0016 JWT header logging) + DES-0006 (Option A sufficiency). `make check` posture unchanged from S-009 close: 143 backend + 203 mobile = **346 tests across 21+ suites — green** (discovery sprint, no code changes per §4.7 + §4.1.1). BLG-0023 / BLG-0024 / BLG-0025 promoted from `drift` to **Ready**. BLG-0026 (umbrella) moved to done. Five new BLGs spawned (BLG-0027 AADE adapter, BLG-0028 Epsilon Net adapter, BLG-0029 Family C identification, BLG-0030 AADE HTML-shape spike, BLG-0032 mobile QR-validator mirror); two post-MVP follow-ups (BLG-0033 cross-source dedup, BLG-0034 HS256 retirement). |
| **Just completed**          | **S-010 — receipt-format scope expansion + auth modernization (discovery).** Resolves both drift findings from the 2026-05-12 live on-device acceptance run. **ADR-0014** (9 participants, no dissent): GR parser becomes a registry-of-adapters within `backend/app/parsers/gr/`; new `is_limited_info` boolean on `ParsedReceipt` + `receipts` schema; Family A AADE in MVP as limited-info adapter; Family B Epsilon Net in MVP as full-SKU adapter; Family C 15-hex non-URL codes pending identification (BLG-0029); two new outbound hosts added (`www1.aade.gr`, `epsilondigital-3rdpartc.epsilonnet.gr`) scoped to parser + spike fetches with §5.8.1 consent; §2.2 / §2.8 / §2.9 amended verbatim per ADR-0014 §6. **ADR-0015** (6 participants, no dissent): supersedes ADR-0002 §1; hand-rolled asymmetric verifier + `cryptography==45.0.1` (one dep, not two); JWKS cache 600s TTL + 60s refetch-floor + hard-fail-401 on unreachable; algorithm allowlist ES256 + RS256 + HS256-transitional; ≥22 tests with ≥95% coverage on `app/auth.py`; mobile coupling via BLG-0024 (silent refresh + retry). **ADR-0016** (4 participants, no dissent): amends ADR-0002 §6; JWT header fields classified as PII-safe public metadata; payloads / signatures / token / `Authorization` value never logged; ≥8 tests including a redaction-regex scan. **DES-0006**: Option A HS256-rollback verified end-to-end (curl loopback 422 + live device 502 — auth gate accepts tokens; the 502 is parser-side, separate from auth). All artifacts under `docs/sprints/S-010-discovery-receipt-format-scope-and-auth-modernization/`. |
| **Last delivered to users** | **None — discovery sprint.** No user-visible behavior shipped in S-010. The S-009 deliverables (on-device receipt scanning under stock Expo Go + PDF export → native share sheet) remain the most recent user-facing changes; the §2.2 / §2.8 / §2.9 wording amendments in this sprint sharpen the documented product story but do not change runtime behavior. |
| **Next sprint**             | **S-011 — implementation (`auth-modernization-and-first-gr-adapter-expansions`)** — Ready queue: BLG-0023 (M, asymmetric JWT verifier — gates the long-term auth posture, lands first), BLG-0024 (S, mobile soft auth-error), BLG-0025 (XS, JWT-header-logging contract + tests, co-located with BLG-0023 in one PR), BLG-0030 (XS-S, AADE HTML-shape spike — can run in parallel), BLG-0032 (S, mobile QR-validator discriminated-union mirror). BLG-0027 (M, AADE adapter) + BLG-0028 (M, Epsilon Net adapter) pulled if BLG-0030 + fixture acquisition resolve in time, otherwise carry to S-012. BLG-0029 (XS, Family C identification) pulled if project-owner photo arrives. Sprint deploy step: rotate Supabase project from "Legacy HS256" back to "JWT Signing Keys (ES256)" after BLG-0023 verified in staging. See `docs/plan.md`. |
