...
| **Current sprint**          | None active — **S-004 (implementation, `login-insights-cache-runnable-scanner`)** just closed green (`make check`: 70 backend + 128 mobile = 198 tests across 13 suites).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Just completed**          | **S-004 — Login + Insights + Encrypted cache + Runnable Scanner (implementation).** All four ADR-anchored Ready items shipped together. **BLG-0012**: Expo SDK 51 runtime tree installed against `package-lock.json`; `tsconfig.json` re-includes the BLG-0003 carve-out; `jest.config.js` runs a two-project layout (`ts` + `jest-expo`); `babel.config.js` added with `babel-preset-expo`. **BLG-0005**: phone-OTP login end-to-end with E.164 normalizer + DES-0002 reducer + `LoginScreen.tsx` against `@supabase/supabase-js`; `db/migrations/0002_handle_new_user.sql` ships the FK + sync trigger per ADR-0004 §3; 14-day refresh tokens. **BLG-0006**: `db/migrations/0003_insights_rpc.sql` (two `plpgsql security invoker` RPCs with explicit `WHERE user_id = user_uuid`), Athens-TZ period helper, `InsightsRepository` interface + in-memory + Supabase impls, `GET /insights/summary` + `GET /insights/products` per ADR-0005 §4 with Bearer JWT, `InsightsScreen.tsx` rendering DES-0003 §3. **BLG-0007**: `CacheRepository` interface + sanitizer (default-deny, `raw_html` dropped) + `InMemoryCacheRepository` (LRU at 200) + `EncryptedAsyncStorageCacheRepository` (AES-256-GCM via `@noble/ciphers/aes` + key in `expo-secure-store`). Three follow-up backlog items filed: BLG-0013 (`tzdata` codification), BLG-0014 (chart-kit re-evaluation), BLG-0015 (live insights-RPC integration test). See `docs/sprints/S-004-implementation-login-insights-cache-runnable-scanner/`. |
| **Last delivered to users** | Login (Supabase native phone OTP), Insights (period totals + by-category + top merchants + top products), encrypted offline cache, runnable Scanner — all Greek-first. The §2.8 MVP is reachable end-to-end with only freelancer-mode (bullets 8 + 9) left to ship. **Note (2026-05-07):** on-device verification of the S-004 UREV acceptance script is currently blocked because Expo Go for iOS only ships SDK 54, while the project is pinned to SDK 51 per ADR-0007. Backend boots cleanly, migrations apply, and `make check` is green; only the on-device path is blocked. Mitigation tracked as **BLG-0016** — ADR debate in S-005, upgrade in S-006.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Next sprint**             | **S-005 — discovery (`freelancer-mode`)**. Settles the contracts for the freelancer flow: tag-as-business UX (inline on Receipt detail vs Profile-level period import), PDF export pipeline (`reportlab` vs `weasyprint` vs server-side puppeteer — `agent-safety-officer` review almost certainly required), inferred-category heuristic (or stay deferred), the BLG-0013 `tzdata` codification (ADR-0007 amendment vs new ADR), and **BLG-0016 — Expo SDK 51 → 54 upgrade ADR** (unblocks on-device verification; co-signed by `agent-safety-officer` for supply-chain review). Output: 4+ ADRs, 1+ DES, ≥ 4 Ready BLGs for S-006 implementation. No production code changed in discovery. See `docs/plan.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
