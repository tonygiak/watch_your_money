...
| **Current sprint**          | None active — **S-006 (implementation, `freelancer-mode-and-sdk-upgrade`)** just closed. `make check`: 143 backend + 197 mobile = **340 tests across 21 suites** (+142 vs. S-005 close baseline of 198 tests across 13 suites). Three of four Ready items shipped at the contract level under SDK 51; one Ready item deferred to S-007.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Just completed**          | **S-006 — Freelancer mode and SDK upgrade (implementation).** Three of four Ready items shipped against ADR-0008 + ADR-0009 + DES-0004 + DES-0005. **BLG-0017**: `PATCH /users/me` Bearer-JWT-protected with server-side ΑΦΜ MOD-11 validation; mobile Profile screen with masked phone, freelancer toggle, ΑΦΜ field, sign-out CTA + cache key rotation per DES-0004 §3.5; full Greek `profile.*` strings; ΑΦΜ MOD-11 validator mirrored in `backend/app/afm.py` + `mobile/src/lib/afm.ts`. **BLG-0018**: `POST /receipts/{id}/tag` per ADR-0008 §2 (Bearer JWT, server-side trim + lowercase on `category` 1..64, idempotent 200, 404 no-enumeration); `tag.state.ts` reducer + `TagPanel.tsx` + `ReceiptDetailScreen.tsx` per DES-0005 §3; Greek `tag.*` + `receipt.*` strings. **BLG-0019**: `GET /export/business-expenses` per ADR-0009 §2 (Bearer JWT, 366-day cap, `StreamingResponse` from in-memory bytes — never persisted, never logged); `reportlab==4.2.5` exact pin; PDF cover + totals + per-receipt rows + page numbers; bundled Bitstream-Vera fonts for monotonic Greek glyphs; control-char + Unicode-bidi-mark sanitizer (PDF-injection defense per ADR-0009 §4); mobile export sub-flow on Profile screen with default-month range, client-side validation, injectable `shareImpl` prop. **Drift — BLG-0016 deferred to S-007** per `AGENTS.md` §4.10: npm registry hung when probing the SDK 54 compat matrix; in-tree SDK 51 pin set + ADR-0006 §2 encryption stack stay byte-identical. Two new follow-up backlog items opened (BLG-0020 share-sheet wiring + BLG-0021 `datetimepicker` swap). See `docs/sprints/S-006-implementation-freelancer-mode-and-sdk-upgrade/`. |
| **Last delivered to users** | At the contract level: **freelancer mode is now end-to-end** — sign in → scan → tag a receipt as a business expense (with category + optional notes; both lowercased / trimmed server-side) → Insights `by_category` rollup picks up the new tag → open Profile → toggle freelancer mode → enter ΑΦΜ (MOD-11 validated client + server) → choose date range → generate PDF (streamed back, never persisted, full Greek glyphs) → sign out (cache key rotated; next user starts clean). **Gated for stock-Expo-Go on-device verification by BLG-0016** — until S-007 ships the SDK 54 upgrade, the §2.8 freelancer-mode acceptance script is exercised at the contract level via `make check` and via dev builds, not on a real Greek consumer's stock Expo Go. The previously-deferred S-004 UREV addendum (Login + Insights + offline cache + Scanner on stock Expo Go) is folded into the same S-007 verification pass. |
| **Next sprint**             | **S-007 — implementation (`sdk-upgrade-and-on-device-acceptance`)**. Pulls **BLG-0016 first** (Expo SDK 51 → 54 upgrade — atomic single-PR `npx expo install --fix` + `expo-doctor` clean + regenerated `mobile/package-lock.json` + the encryption-stack round-trip test, forward-only variant from S-005 plan §5; `react-native-chart-kit` survival absorbed if needed via BLG-0014), then **BLG-0020** (fold `expo-sharing` + `expo-file-system` from the SDK 54 expected matrix into the Profile-export `shareImpl` prop) and **BLG-0021** (replace plain `TextInput` date entry with `@react-native-community/datetimepicker`) in parallel. `S-007-UREV-0001` runs the full `S-006-UREV-0001` §A freelancer-mode script **and** the previously-deferred S-004 UREV addendum on stock Expo Go end-to-end. After S-007, every bullet in `AGENTS.md` §2.8 is shippable on a real Greek consumer's phone. See `docs/plan.md`. |
