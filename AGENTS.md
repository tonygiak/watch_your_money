...
| **Current sprint**          | None active — **S-009 (implementation, `sdk-upgrade-and-on-device-acceptance-v2`)** just closed. `make check`: 143 backend + 203 mobile = **346 tests across 21+ suites** under the new `jest-expo@54.0.17` + `react@19.1.0` + `react-native@0.81.5` tree (identical count to S-007 / S-008 — the contract surface didn't change; only the runtime tree moved underneath). BLG-0016 + BLG-0020 + BLG-0021 done. ADR-0013 closed (purpose served). |
| **Just completed**          | **S-009 — SDK upgrade and on-device acceptance v2 (implementation).** The three-sprint Expo SDK 51 → 54 upgrade landed. ADR-0013 §3 pre-flight executed: Step 1 (Node v22.22.0) passed; Step 2 skipped (already on LTS); Step 3 TLS smoke test **failed even on Node v22** with `UNABLE_TO_VERIFY_LEAF_SIGNATURE`; Step 3a (export Windows OS-managed CA bundle to `~/ca-bundle.pem` + `NODE_EXTRA_CA_CERTS`) **passed the retry on the first try** — 62 root CAs, 97208-byte PEM, never committed, fully reversible. Step 4 (`npx expo install --fix` + `expo-doctor`) clean: 17/17 checks passed. Both ADR-0012 §3 deviations closed (`@react-native-community/netinfo@11.4.1`, `typescript@5.9.2`). Three drift candidates resolved in-sprint as runtime-tree mechanics with no contract changes: (a) `mobile/tsconfig.json` `moduleResolution: "node"` override removed; (b) eight `JSX.Element` → `React.JSX.Element` (React 19 removed the global `JSX` namespace); (c) `babel-preset-expo@54.0.10` + `expo-modules-core@3.0.30` promoted to direct devDependencies (SDK 54 + npm 10 hoisting nests them under `expo/node_modules/`). Encryption round-trip from S-007 stayed byte-identical under SDK 54's `@noble/ciphers@0.5.3`. `react-native-chart-kit@6.12.0` survived — BLG-0014 stays passive per ADR-0012 §6. Option B (EAS dev client) remains deferred per ADR-0012 §1 / ADR-0013 §4. See `docs/sprints/S-009-implementation-sdk-upgrade-and-on-device-acceptance-v2/`. |
| **Last delivered to users** | The §2.8 MVP bullets 4 (on-device receipt scanning under stock Expo Go) and 9 (PDF export → native share sheet) are now **reachable on a real Greek consumer's phone**. Verification path documented end-to-end in `S-009-UREV-0001` (sign in → scan → Insights → offline → restore + freelancer-mode tag → ΑΦΜ → export PDF → native share-sheet hand-off + native date-picker). All BLG-0020 + BLG-0021 acceptance bullets at the on-device-resolution level land in this sprint. |
| **Next sprint**             | **S-010 — likely discovery** — chaired by `orchestrator` with `product-owner` + `product-manager` + `architect` + `data-architect` + `parser-specialist` + `localization-specialist` + `agent-safety-officer`. The Ready queue is empty for delivery work. Discovery questions: (a) which of `BLG-0004` (real-receipt fixtures) / `BLG-0009` (CI drift hook) / `BLG-0011` (language switch) / `BLG-0015` (live insights RPC test) is closest to Ready? (b) is the door open for an EU country expansion discovery (RO / IT / PT / ES adapter per §5.9 + ADR-0001)? (c) does the §2.9 out-of-scope list need a fresh review now that the SDK 54 tree unblocks on-device acceptance? See `docs/plan.md`. |
