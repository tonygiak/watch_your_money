...
- **SKU‑level** receipt data (not just merchant and total)
...
- **Encryption-stack round-trip test** — `mobile/__tests__/cache/encryption.roundtrip.test.ts` — six-case AES-256-GCM round-trip via `@noble/ciphers/aes` `gcm()` directly (Greek UTF-8 round-trip, deterministic key + IV round-trip, GCM tag mismatch under wrong key, tampered-ciphertext rejection via auth-tag, `randomBytes` length sanity). Forward-only variant per S-005 plan §5: runs under whatever `@noble/ciphers` resolution is loaded — today SDK 51's `0.5.3`, tomorrow SDK 54's resolution — and catches a regression in either direction so the offline-cache encryption contract from ADR-0006 §2 / ADR-0012 §5 cannot regress silently across an SDK upgrade. *(BLG-0016 acceptance bullet 5 partial, S-007)*
...
| **Current sprint**          | None active — **S-007 (implementation, `sdk-upgrade-and-on-device-acceptance`)** just closed. `make check`: 143 backend + 203 mobile = **346 tests across 21+ suites** (+6 vs. S-006 close baseline of 340 tests across 21 suites). Two of three Ready items shipped at the contract level (BLG-0020, BLG-0021) plus the BLG-0016 acceptance bullet 5 partial (encryption-stack round-trip test); BLG-0016 itself **deferred for the third sprint running** and **escalated** to S-008 discovery per `AGENTS.md` §4.10.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Just completed**          | **S-007 — SDK upgrade and on-device acceptance (implementation).** Two of three Ready items shipped at the contract level + the BLG-0016 acceptance bullet 5 partial. **BLG-0020**: `mobile/src/lib/share.ts` ships `defaultShareImpl({ base64, filename })` — lazy-requires `expo-file-system` + `expo-sharing` at call time, writes the base64 PDF to the cache directory, opens the native share sheet via `shareAsync(uri, { mimeType: "application/pdf" })`; `ProfileScreen.tsx` falls back to `defaultShareImpl` when the prop is omitted; tests still inject a fake; privacy contract from ADR-0009 §3 honored byte-identically. **BLG-0021**: `mobile/src/screens/profile/DateField.tsx` wraps `@react-native-community/datetimepicker` behind a `loadPicker()` try / catch — under SDK 54 the require resolves and the native picker opens; under SDK 51 the require returns null and the component renders just a trigger button; the reducer is unchanged at the action / state level. `mobile/jest.config.js` `transformIgnorePatterns` includes the new SDK 54 deps. **BLG-0016 partial**: `mobile/__tests__/cache/encryption.roundtrip.test.ts` — six-case AES-256-GCM round-trip test via `@noble/ciphers/aes` `gcm()` directly (Greek UTF-8 round-trip, deterministic key + IV round-trip, GCM tag mismatch + tampered-ciphertext rejection, `randomBytes` length sanity). Forward-only variant per S-005 plan §5: runs under whatever `@noble/ciphers` resolution is loaded — today SDK 51's `0.5.3`, tomorrow SDK 54's resolution; catches a regression in either direction. **Drift — BLG-0016 third deferral, escalated**: two install attempts hit `UNABLE_TO_VERIFY_LEAF_SIGNATURE` against `registry.npmjs.org` for the SDK 54 tree (TLS chain validation failing). The principled options ruled out per `agent-runtime-security.md`: `strict-ssl=false`, third-party registry mirror. The third deferral on the same outbound surface escalates to S-008 discovery (BLG-0022 opened). See `docs/sprints/S-007-implementation-sdk-upgrade-and-on-device-acceptance/`. |
| **Last delivered to users** | At the contract level: **the share-sheet hand-off and native date picker for the freelancer-mode export are now wired** — when SDK 54 lands (S-009 implementation), tapping `Δημιουργία PDF` on the Profile screen opens the iOS / Android share sheet with the streamed PDF; the two date inputs swap from plain `TextInput`s to native pickers that localize to the device locale automatically. The encryption-stack round-trip test ships today and proves the AES-256-GCM contract from ADR-0006 §2 + ADR-0012 §5 holds under whatever `@noble/ciphers` resolution is loaded. **Still gated for stock-Expo-Go on-device verification by BLG-0016** — the third deferral means the §2.8 freelancer-mode acceptance script + the S-004 UREV addendum stay reachable only through dev builds (SDK 51 tree); stock Expo Go users (which on the iOS App Store / Google Play Store now require SDK 54+) cannot run the app end-to-end until S-009 ships the upgrade. |
| **Next sprint**             | **S-008 — discovery (`sdk-upgrade-path-forward`)** — chaired by `orchestrator` with `agent-safety-officer` + `architect` + `engineering-manager` + `mobile-builder` + `devops-engineer`. Output: an ADR (likely `ADR-0013`, or an `ADR-0012` amendment per `AGENTS.md` §4.4 supersession rules) that resolves the BLG-0016 third-deferral so the upgrade can actually run in S-009 implementation. Three options on the table: (a) network-environment fix (different machine / network, Verdaccio mirror, pre-validated tarball cache flow), (b) ADR-0012 §1 amendment toward Strategy 3 (EAS dev client / TestFlight, accepting the operational shift originally rejected — three deferrals is enough signal to revisit), (c) split-into-two-upgrade approach (SDK 54 dev-client first, full Expo Go a separate sprint). Decision recorded with `agent-safety-officer` co-sign on the supply-chain implications of the chosen path. After S-008, S-009 implementation pulls BLG-0016 + the on-device acceptance for BLG-0020 / BLG-0021. See `docs/plan.md`. |
...
3. A user can scan a Greek e‑invoice QR code from any Entersoft or SoftOne merchant.
4. The full receipt with all line items appears in their app within **5 seconds**.
...
OCR fallback for non‑QR receipts; card linkage and automatic receipt detection; loyalty points or rewards; social features; B2B intelligence dashboard; multi‑currency support; multi‑country support (Greece only for now, but architecture must allow future expansion); bank account integration; notifications and reminders; subscription tracking; budget setting and alerts; receipt sharing between users.
