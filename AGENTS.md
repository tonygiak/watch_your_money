...
- **Scanner state machine + on‑device QR validator** — pure‑TS reducer covering every DES‑0001 transition, with Greek‑first i18n + Greek‑first locale detection. The on‑device GR QR validator mirrors the backend regex for defense in depth. The runnable `ScannerScreen.tsx` is scaffolded against `expo-camera`/`AbortController` and lights up as soon as BLG‑0012 ships the Expo runtime tree. *(BLG‑0003 testable parts, S‑002)*
- **CI** — GitHub Actions runs `make check` on every push and PR (backend lint + typecheck + tests, mobile typecheck + tests, fixture‑driven parser tests). *(BLG‑0008, S‑002)*
...
| **Current sprint**          | None active — **S-002 (implementation, `scan-and-store`)** just closed green (`make check`: 38 backend + 52 mobile = 90 tests).                                                                                                                                                                                                                                                                                                                                                            |
| **Just completed**          | **S-002 — Scan and store (implementation).** Shipped the full Greek e‑receipt parser (BLG-0001), `POST /receipts/parse` with stdlib‑only HS256 JWT, `(user_id, mark)` idempotency, RFC‑7807 errors and a privacy gate (BLG-0002), the testable parts of the Scanner stack — reducer + validator + i18n + locale detection (BLG-0003), one synthetic GR fixture triplet (BLG-0004 partial), and CI on push/PR (BLG-0008). One drift item BLG-0012 added — install the Expo runtime tree under proper safety review and wire `ScannerScreen.tsx` into `make check`. See `docs/sprints/S-002-implementation-scan-and-store/`. |
| **Last delivered to users** | First user‑visible behavior just shipped: Greek e‑receipt parser + `POST /receipts/parse` end‑to‑end + Scanner state machine + Greek‑first i18n. The runnable scanner screen lands in S-004 once BLG-0012's runtime install is approved.                                                                                                                                                                                                                                                  |
| **Next sprint**             | **S-003 — discovery (`auth-and-cache`)**. Settle ADRs for phone OTP (BLG-0005), insights computation (BLG-0006), offline cache strategy (BLG-0007), and the Expo runtime tree (BLG-0012). Reconcile §5.3.2 wording with ADR-0002 (BLG-0010). Output: Ready items for the next implementation sprint S-004 (login + insights + cache + runnable scanner). See `docs/plan.md`.                                                                                                              |
...
- **POST `/receipts/parse`** — Input `{ "qr_url": string, "user_id": string }`. Fetches HTML from the QR URL, parses receipt, stores in DB. Returns structured receipt JSON with all line items.
- **GET `/receipts`** — Query: `user_id`, optional `from_date`, `to_date`, `category`, `merchant`. Returns list of receipts for that user.
- **GET `/receipts/{receipt_id}`** — Returns single receipt with all line items.
- **GET `/insights/summary`** — Query: `user_id`, `period` (week / month / year). Returns spending totals, by category, by merchant, vs previous period.
- **GET `/insights/products`** — Query: `user_id`, `period`. Returns most purchased products with frequency, total spend, average price.
- **POST `/receipts/{receipt_id}/tag`** — Input `{ "is_business": boolean, "category": string, "notes": string }`. Tags a receipt for freelancer business‑expense tracking.
- **GET `/export/business-expenses`** — Query: `user_id`, `from_date`, `to_date`. Returns PDF of all business‑tagged receipts for accountant submission.
