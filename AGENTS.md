...
| **Current sprint**          | None active — **S-003 (discovery, `auth-and-cache`)** just closed green (`make check`: 38 backend + 52 mobile = 90 tests, smoke check; no production code changed in a discovery sprint).                                                                                                                                                                                                                                                                                                                                                            |
| **Just completed**          | **S-003 — Auth and cache (discovery).** Settled the four contracts that gate every S-004 user-visible item: **ADR-0004** (Supabase native phone OTP, no new outbound surface, 14-day refresh tokens, explicit Twilio rejection), **ADR-0005** (Insights via PostgREST RPCs + FastAPI orchestration with Athens-TZ boundaries; decimal-as-string responses), **ADR-0006** (encrypted offline cache: AsyncStorage + AES-256-GCM via `@noble/ciphers` + key in `expo-secure-store`; LRU cap 200), **ADR-0007** (Expo SDK 51 with 17 runtime + 6 dev exact-pinned packages, supply-chain review captured verbatim by `agent-safety-officer`). Plus **DES-0002** (Login UX), **DES-0003** (Insights UX), and **BLG-0010 closed** — `AGENTS.md` §5.3.2 reconciled with ADR-0002 + ADR-0005. Four backlog items moved to **Ready** for S-004: BLG-0005 (Login + OTP), BLG-0006 (Insights endpoints + screen), BLG-0007 (Encrypted offline cache), BLG-0012 (Expo runtime install + gate re-inclusion). See `docs/sprints/S-003-discovery-auth-and-cache/`. |
| **Last delivered to users** | Unchanged from S-002: Greek e‑receipt parser + `POST /receipts/parse` end‑to‑end + Scanner state machine + Greek‑first i18n. Discovery sprints don't ship user-visible behavior; the next user-visible wave (login, insights, encrypted offline cache, runnable scanner) lands in S-004.                                                                                                                                                                                                                                                  |
| **Next sprint**             | **S-004 — implementation (`login-insights-cache-runnable-scanner`)**. Pulls all four Ready items: BLG-0012 first (install the pinned Expo tree + lift the gate carve-out), then BLG-0005 (Supabase OTP login + phone normalizer + `auth.users` ↔ `public.users` trigger), BLG-0006 (insights RPCs + FastAPI endpoints + Insights screen), BLG-0007 (encrypted AsyncStorage cache + offline UX). Acceptance test: real Greek user installs, signs in with `+30` phone, scans a receipt in ≤ 5 s, opens Insights, kills network, still sees cached list with offline banner. See `docs/plan.md`.                                                                                                              |
