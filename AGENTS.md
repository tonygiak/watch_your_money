...
| **Current sprint**          | None active — **S-011 (implementation, `auth-modernization-and-jwt-header-logging-contract`)** just closed. **BLG-0023 + BLG-0024 + BLG-0025 all shipped in one PR** per the ADR-0015 §8 + ADR-0016 §3 hard-ordering constraint. `make check`: **389 tests across 21+ suites — green** (346 → 389, +43: +30 in `tests/auth/test_jwt.py`, +9 in `tests/test_auth_logging.py`, +4 in `mobile/__tests__/screens/scanner/state.test.ts`). Ready queue carried to S-012: BLG-0030 (AADE HTML-shape spike), BLG-0027 (AADE adapter), BLG-0028 (Epsilon Net adapter), BLG-0032 (mobile QR-validator mirror) — all gated on consented fixtures or on each other. Two new runbooks shipped (`rotate-supabase-jwt-signing-keys`, `rollback-to-hs256-only`). BLG-0034 (HS256 retirement) stays planned until BLG-0023 runs one production release cycle. |
| **Just completed**          | **S-011 — auth modernization + JWT-header-logging contract (implementation).** **BLG-0023**: `backend/app/auth.py` rewritten end-to-end per ADR-0015. Hand-rolled asymmetric verifier covering ES256 + RS256 + HS256-transitional with a strict algorithm allowlist + a `(alg, key-type)` cross-check matrix rejecting `RS256/EC` and `ES256/RSA` mismatches. `alg=none`, unknown KIDs, bad signatures, expired / wrong-audience / wrong-subject claims all rejected via the existing RFC-7807 envelope. `CachedJwksProvider` ships in `app/auth.py` (600 s TTL + 60 s refetch-floor + stale-on-failure-when-cached + hard-fail-401-when-uncached); `app/services/jwks_provider.py` wires the singleton into FastAPI DI. One new core dep: `cryptography==45.0.1` (PyJWT stayed rejected). Five route files (`receipts`, `insights`, `receipt_tag`, `users`, `exports`) migrated to the new DI pattern: each keeps `get_jwt_secret` (returning the legacy HS256 secret for test override stability) and adds `get_jwks_provider`; `require_authenticated_user` calls `verify_supabase_jwt(token, jwks_provider=…, legacy_hs256_secret=…)`. New config vars: `SUPABASE_JWKS_URL`, `SUPABASE_JWKS_CACHE_TTL_SECONDS`, `SUPABASE_JWT_LEGACY_HS256_SECRET` (with boot-time conflict detection against the deprecated `SUPABASE_JWT_SECRET`). **BLG-0024**: scanner reducer (`mobile/src/screens/scanner/state.ts`) gains `auth_error_recoverable` + `auth_error_terminal` states + `hasAttemptedAuthRefresh` flag + `RETRY_AFTER_REFRESH` action; the first 401 silently calls `supabase.auth.refreshSession()` (wired through `App.tsx`) and re-attempts the parse before the terminal sign-out path; new string `scanner.error.auth.refreshing` (el + en). **BLG-0025**: `backend/tests/test_auth_logging.py` (new, 9 tests) implements the ADR-0016 §3 contract — exactly one log line per `JwtError` with `code` + `alg` + `typ` + `kid`-truncated-to-`first 6 chars + "…"` + static `reason`; malformed tokens log `None` for header fields without crashing; **redaction regex** scans every captured log record to prove the full token, the `Bearer ` prefix, the payload claims (email / phone / sub), and the raw `Authorization` value never appear in logs. ADR-0002 §1 superseded by ADR-0015; ADR-0002 §6 amended by ADR-0016. All artifacts under `docs/sprints/S-011-implementation-auth-modernization-and-jwt-header-logging-contract/`. |
| **Last delivered to users** | **The first 401 on a scan no longer signs the user out.** Mobile clients now silently refresh the Supabase session once on a recoverable auth error before the terminal sign-out path (BLG-0024). Backend-side, the asymmetric verifier (BLG-0023) is live behind the same RFC-7807 envelope users already saw — no user-visible API surface change. The Supabase project still runs on Legacy HS256 keys today; operator rotation to ES256/JWKS lands in the S-012 deploy window per `docs/runbooks/rotate-supabase-jwt-signing-keys.md`. |
| **Next sprint**             | **S-012 — implementation (`first-gr-adapter-expansions`)** — Ready queue: BLG-0030 (XS-S, AADE HTML-shape spike — lands first, gates BLG-0027), BLG-0027 (M, AADE limited-info adapter), BLG-0028 (M, Epsilon Net full-SKU adapter), BLG-0032 (S, mobile QR-validator discriminated-union mirror — can ship shape ahead of either adapter). BLG-0029 (XS, Family C identification) pulled if project-owner photo arrives. Sprint deploy step: rotate Supabase project from "Legacy HS256" back to "JWT Signing Keys (ES256)" per `docs/runbooks/rotate-supabase-jwt-signing-keys.md` now that BLG-0023 is live and ≥95% covered. See `docs/plan.md`. |
