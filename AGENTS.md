...
| **Current sprint**          | None active — **S-005 (discovery, `freelancer-mode`)** just closed (`make check`: 70 backend + 128 mobile = 198 tests across 13 suites — same as S-004 close; the only production-code touch was a comment-only update in `backend/requirements.txt` for BLG-0013, byte-identical pin).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Just completed**          | **S-005 — Freelancer mode (discovery).** Five ADRs decided, two design artifacts written, four backlog items refined to **Ready** for S-006, one item (BLG-0013) closed in-sprint via the ADR-0011 admin edit. **ADR-0008**: inline tag-on-detail is the MVP critical path; `POST /receipts/{id}/tag` Bearer-JWT-protected; free-text `category` lowercased server-side. **ADR-0009**: `reportlab==4.2.5` for PDF export — pure-Python, no system deps, no new outbound surface, full Greek glyphs via `DejaVuSans`; PDF on-the-fly + `StreamingResponse`, never persisted, never logged; `weasyprint` rejected (Cairo / Pango / GTK Dockerfile cost); server-side `puppeteer` rejected (would have added `storage.googleapis.com` to the allowlist). **ADR-0010**: inferred-category heuristic stays **deferred** with three concrete re-evaluation triggers; LLM-API call directly forbidden by §2.4. **ADR-0011**: standalone ADR codifying `tzdata==2024.2` in `backend/requirements.txt`. **ADR-0012**: target Expo SDK 54; supersedes ADR-0007 §2 only; `npx expo install --fix` + `expo-doctor` + atomic single-PR commit in S-006; both in-tree compat-matrix warnings (`netinfo`, `typescript`) re-aligned to the SDK 54 matrix; encryption stack from ADR-0006 must survive byte-identically. **DES-0004**: Profile screen with freelancer toggle + ΑΦΜ field (Greek MOD-11 validator) + PDF export action. **DES-0005**: inline tag-as-business flow on Receipt detail. See `docs/sprints/S-005-discovery-freelancer-mode/`. |
| **Last delivered to users** | Login (Supabase native phone OTP), Insights (period totals + by-category + top merchants + top products), encrypted offline cache, runnable Scanner — all Greek-first (unchanged from S-004 — discovery sprints ship no user-visible behavior). The §2.8 MVP is reachable end-to-end with only freelancer-mode (bullets 8 + 9) and the SDK-54-upgrade unblocker left for S-006. **Note (2026-05-07):** on-device verification of the S-004 UREV acceptance script remains blocked until BLG-0016 ships in S-006. The contracts are now locked in ADR-0008..0012 + DES-0004..0005; `make check` is green; the BLG-0013 audit-trail gap is closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Next sprint**             | **S-006 — implementation (`freelancer-mode-and-sdk-upgrade`)**. Pulls four Ready items: **BLG-0016 first** (Expo SDK 51 → 54 upgrade — unblocks on-device verification on stock Expo Go, includes the encryption-stack round-trip test against ADR-0006 §2), then **BLG-0017** (Profile screen + freelancer toggle + ΑΦΜ field + sign-out, mobile + thin `PATCH /users/me`), **BLG-0018** (`POST /receipts/{id}/tag` endpoint + Receipt-detail tag UX per DES-0005), **BLG-0019** (`GET /export/business-expenses` PDF endpoint via `reportlab` + Profile export action per DES-0004 §3.4). At sprint review: a Greek freelancer with stock Expo Go runs the full §2.8 acceptance script (sign in → scan → tag as business → Insights → Profile → ΑΦΜ → export PDF → share). After S-006, every bullet in §2.8 is shippable. See `docs/plan.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
