...
| **Current sprint**          | None active — **S-008 (discovery, `sdk-upgrade-path-forward`)** just closed. `make check`: 143 backend + 203 mobile = **346 tests across 21+ suites** (unchanged from S-007 — no production code changes in a discovery sprint). ADR-0013 accepted; BLG-0022 done; BLG-0016 unblocked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Just completed**          | **S-008 — SDK upgrade path forward (discovery).** Single output: **ADR-0013 accepted**. The three-sprint `UNABLE_TO_VERIFY_LEAF_SIGNATURE` blocker was diagnosed as a **Node.js CA bundle staleness issue**: Node.js ships its own Mozilla CA store; when `registry.npmjs.org` adopted a newer root/intermediate CA after the installed Node.js version was released, TLS validation fails. Three options debated — (A) fix the host CA environment, (B) EAS dev client, (C) split upgrade — all five participants (`agent-safety-officer`, `architect`, `engineering-manager`, `mobile-builder`, `devops-engineer`) converged on **Option A** in Round 1 with no dissent in Round 3. Decision: update Node.js to v22 LTS and/or export the Windows system CA trust store via `NODE_EXTRA_CA_CERTS` — both keep `strict-ssl` fully enabled, require no new outbound host, and preserve `agent-runtime-security.md` byte-identically. Concrete pre-flight checklist locked in ADR-0013 §3. ADR-0012 §1 (EAS dev client rejection) remains in force unless S-009 exhausts the checklist. **BLG-0016 updated to "Ready, executable per ADR-0013 §3."** BLG-0022 completed. See `docs/sprints/S-008-discovery-sdk-upgrade-path-forward/`. |
| **Last delivered to users** | No new user-visible behavior in S-008 (discovery sprint). Feature catalog unchanged from S-007. **Still gated for stock-Expo-Go on-device verification by BLG-0016** — the §2.8 freelancer-mode acceptance script + the S-004 UREV addendum remain reachable only through dev builds until S-009 ships the SDK 54 upgrade. |
| **Next sprint**             | **S-009 — implementation (`sdk-upgrade-and-on-device-acceptance-v2`)** — pulls BLG-0016 first per the ADR-0013 §3 pre-flight checklist (Node.js v22 LTS update → TLS smoke test → `npx expo install --fix` per ADR-0012 §2); then on-device verification of BLG-0020 (share-sheet hand-off) + BLG-0021 (native date-picker). Runs `S-009-UREV-0001` end-to-end on stock Expo Go once the SDK 54 tree is live. If the pre-flight checklist is fully exhausted without success, opens BLG-0023 and S-010 for Option B (EAS dev client). See `docs/plan.md`. |
