...
| **Current sprint**          | None active — **S-012 (implementation, `mobile-qr-validator-shape`)** closed earlier; **post-S-012-close drift findings recorded 2026-05-12 evening** at `docs/spikes/gr-qr-ground-truth-2026-05-12/findings.md`. Two real Greek receipts (one AADE tameiakí, one Epsilon Net) arrived from the project owner under explicit `AGENTS.md` §5.8.1 consent, plus the **QR-decoded URL strings** from both. The decodes revealed two regex-shape drifts from ADR-0014 §3: the AADE SIG charset is `[A-Z0-9]+(\.[0-9]+)?`, not hex-only; the Epsilon path is `/DocViewer/<uuid>`, not `/fd/<hash>:<n>`. Both corrections are captured as **BLG-0035 + BLG-0036**, Ready for S-013, no upstream fetch required. `make check` posture is unchanged from S-012 close (**411 tests, green**) — these are post-sprint planning artifacts per §4.1.5, not new code yet. |
...
| **Next sprint**             | **S-013 — implementation (`gr-validator-drift-corrections`)** — pulls BLG-0035 + BLG-0036 in one PR. Both are XS, both touch only `mobile/src/parsers/gr.ts` + `mobile/__tests__/parsers/gr.test.ts`, both share one ADR-0014 §3 amendment block, and **neither requires an upstream fetch** — the ground-truth QR strings under §5.8.1 consent are already on disk at `docs/spikes/gr-qr-ground-truth-2026-05-12/findings.md`. The originally-proposed S-013 theme `first-gr-adapter-expansions` defers to **S-014** because the backend adapters (BLG-0027 + BLG-0028 + BLG-0030) still need the gated AADE / Epsilon `raw.html` fetches with `agent-safety-officer` + `security-privacy-officer` ToS-review sign-off per ADR-0014 §4. Sprint deploy step (independent of S-013 scope): rotate Supabase project from "Legacy HS256" back to "JWT Signing Keys (ES256)" per `docs/runbooks/rotate-supabase-jwt-signing-keys.md` (still pending from S-012). See `docs/plan.md`. |
